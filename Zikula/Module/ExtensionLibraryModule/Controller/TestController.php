...
class TestController extends \Zikula_AbstractController
{
    ...
    public function postReceiveHookAction($type = 0)
    {
        ...
        $url = $this->request->getUriForPath($this->get('router')->generate('zikulaextensionlibrarymodule_post_processinbound'));
        switch ($type) {
            case 10:
    ...
    public function importImage($type = 'valid') {
        ...
        if ($imageManager->import()) {
            $this->view->assign('src', $this->get('router')->generate('zikulaextensionlibrarymodule_user_getimage', array('name' => $imageManager->getName())));
        } else {
            $this->view->assign('src',  $this->get('router')->generate('zikulaextensionlibrarymodule_user_getimage'));
        }
